...
class LevelUpForm extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      passivePerception: this.props.character.passivePerception ? this.props.character.passivePerception : 0,
      passivePerceptionValid: true,
      maxHp: this.props.character.maxHp ? this.props.character.maxHp : 0,
      maxHpValid: true,
      subclassName: '', subclassValid: false,
      errorMsg: {},
      formValid: false
    }
  }
  ...
  onClassFormChange = (subclass) => {
    this.setState({subclassName: subclass.subclassName, subclassValid: subclass.subclassValid}, this.validateForm)
  }

  validateForm = () => {
    const { passivePerceptionValid, maxHpValid, subclassValid } = this.state
    this.setState({formValid: passivePerceptionValid && maxHpValid && subclassValid})
  }
      ...
      <Container>
        <Form onSubmit={this.onSubmit}>
          ...
          <Modal.Footer>
            ...
            <div>
              Class/Subclass: {this.state.subclassValid ? <i className='valid-input icon-ok-circle' /> : <i className='invalid-input icon-remove-sign' />}
            </div>

            <Button disabled={!this.state.formValid} type="submit">Level Up</Button>
          </Modal.Footer>
        </Form>
      </Container>
    )
