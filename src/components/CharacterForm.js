...
class CharacterForm extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      firstName: '', fistNameValid: false,
      lastName: '',
      raceName: '', raceNameValid: false,
      armorClass: 1, armorClassValid: true,
      passivePerception: 1, passivePerceptionValid: true,
      maxHp: 1, maxHpValid: true,
      gold: 0, goldValid: false,
      subclassName: '', subclassNameValid: false,
      altVision: '', altVisionValid: false,
      errorMsg: {},
      formValid: false
    }
  }
  ...
  onClassFormChange = (subclass) => {
    this.setState({subclassName: subclass.subclassName, subclassNameValid: subclass.subclassValid}, this.validateForm)
  }

  validateForm = () => {
    const { firstNameValid, armorClassValid, raceNameValid, passivePerceptionValid, maxHpValid, subclassNameValid, goldValid, altVisionValid} = this.state
    this.setState(
      {
        formValid: 
          firstNameValid && 
  ...
  render() {
    ...
    const altVisionChoices = [{value:'NORM', label: 'Normal Vision'}, {value:'60DV', label: '60 ft Dark Vision'}]
      ...
      <Container>
        <Form onSubmit={this.onSubmit}>
          <Modal.Body>
            ...
            />
            <ValidationMessage valid={this.state.armorClassValid} message={this.state.errorMsg.armorClass} />
            ...
            />
            <ValidationMessage valid={this.state.passivePerceptionValid} message={this.state.errorMsg.passivePerception} />
            ...
            <ValidationMessage valid={this.state.altVisionValid} message={this.state.errorMsg.altVision} />
            ...
            {this.props.racesIsLoading ? <p>loading races....</p> :
              <Select options={this.getRaceOptions()} onChange={this.onRaceChange} />
            }
              <ValidationMessage valid={this.state.raceNameValid} message={this.state.errorMsg.raceName} />
            
            {this.props.subclassesIsLoading ? <p>loading subclasses....</p> :
              <ClassForm subclasses={this.props.subclasses} onChange={this.onClassFormChange}/>
            }
          ...
          <Modal.Footer>
            ...
            <div>
              First Name: {this.state.firstNameValid ? <i className='valid-input icon-ok-circle' /> : <i className='invalid-input icon-remove-sign' />}
            </div> 
            <div>
              Last Name is Optional: <i className='valid-input icon-ok-circle' />
            </div>  
            <div>
              Armor Class: {this.state.armorClassValid ? <i className='valid-input icon-ok-circle' /> : <i className='invalid-input icon-remove-sign' />}
            </div>  
            <div>
              Passive Perception: {this.state.passivePerceptionValid ? <i className='valid-input icon-ok-circle' /> : <i className='invalid-input icon-remove-sign' />}
            </div>  
            <div>
              Max HP: {this.state.maxHpValid ? <i className='valid-input icon-ok-circle' /> : <i className='invalid-input icon-remove-sign' />}
            </div>  
            <div>
              Vision: {this.state.altVisionValid ? <i className='valid-input icon-ok-circle' /> : <i className='invalid-input icon-remove-sign' />}
            </div>  
            <div>
              Starting Gold: {this.state.goldValid ? <i className='valid-input icon-ok-circle' /> : <i className='invalid-input icon-remove-sign' />}
            </div>
            <div>
              Race: {this.state.raceNameValid ? <i className='valid-input icon-ok-circle' /> : <i className='invalid-input icon-remove-sign' />}
            </div>  
            <div>
              Class: {this.state.subclassNameValid ? <i className='valid-input icon-ok-circle' /> : <i className='invalid-input icon-remove-sign' />}
            </div> 
            <Button variant ="primary" disabled={!this.state.formValid} type="submit">Add Character</Button>
          </Modal.Footer>
        </Form>
      </Container>
    )
