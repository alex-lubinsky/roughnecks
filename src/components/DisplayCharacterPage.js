...
import { startSetCharacters, startUpdateCharacter } from '../actions/characters';
      ...
      <div>
        ...
        <div>
            <Modal show={this.state.showKillPCModal} onHide={this.handleKillPCClose} >
                ...
                <Container>
                  <Button onClick={this.killPcConfirm} variant="danger">
                    Yes, {this.props.character.firstName} {this.props.character.lastName} died a glorious death in battle.
                  </Button>
                  <Button onClick={this.handleKillPCClose} variant="link">....Nevermind</Button>
                </Container>
            </Modal>
            ...
            getLevel(this.props.missions.filter(mission => 
              mission.characters.some(character => character === this.props.characterid)
            ).length + this.props.missions.filter(mission => mission.dm === this.props.characterid).length) 
            && this.props.character.creator === this.props.userid)  ? 
            <Button variant="success" onClick={this.handleLevelUpShow}>Level Up!</Button> : null}

            {this.props.character.creator === this.props.userid ? <Button onClick={this.handleKillPCShow} variant="danger">Character Died</Button> : null}
            <Row>
              <Col>
              <Container>
                <p>Race: {this.props.races.find(race => race.id === this.props.character.raceName).raceName}</p>
              ...
              <Container>
              Class: <ClassBuilder 
                pcClasses={this.props.pcSubclasses.filter(pcSubclass => pcSubclass.classCharacter === this.props.characterid)} 
            ...
            Missions:
            {this.props.missions.filter(mission => mission.characters.some(character => character === this.props.characterid))
              .map((mission) => {
                return <p key={mission.id}>{mission.name}</p>
              })
            }
            DMed:
            {this.props.missions.filter(mission => mission.dm === this.props.characterid).map((dm) => {
              return <p key={dm.id}>{dm.name}</p>
            })}
            Earned: <TotalEarnedMoney transactions={this.props.transactions.filter(transaction => 
              transaction.characters.some(character => character === this.props.characterid)
            )} />
            Spent: <TotalSpentMoney transactions={this.props.transactions.filter(transaction => 
              transaction.characters.some(character => character === this.props.characterid)
            )} />
            Balance: <TotalBalanceMoney transactions={this.props.transactions.filter(transaction => 
              transaction.characters.some(character => character === this.props.characterid)
            )} />
            <TransactionsTable 
...
const mapStateToProps = (state, props) => ({
  ...
  character: state.characters.data.find(character => character.id.toString() === props.match.params.id),
...
export default connect(mapStateToProps,mapDispatchToProps)(DisplayCharacterPage);